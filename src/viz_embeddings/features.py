"""Extract embeddings featuares to files
...
def unique_vocabulary(vocab, embeddings, pooling, do_lower):
    ...
    """
    unique_vocab = (
        list(set([term.lower() for term in vocab])) if do_lower else list(set(vocab))
    )

    compare = lambda x, y: x.lower() == y.lower() if do_lower else x == y
...
def extract_embeddings(
    ...
    doc_lst = [
        words[k * doc_length : min((k + 1) * doc_length, len(words))]
    ...
    try:
        for doc in doc_lst:
            doc_vocab, doc_embeddings = vocabulary_and_embeddings(
                doc, tokenizer, model, level_name, pooling
            )
    ...
    if unique:
        vocab, embeddings = unique_vocabulary(vocab, embeddings, pooling, do_lower)

    if filter_func:
        vocab, embeddings = filter_vocabulary(vocab, embeddings, filter_func)
